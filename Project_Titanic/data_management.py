...
# Function to clean/prune titanic data.
def clean_titanic2(titanic_data):
    """ Fixes missing values and removes unnessary data.
    ...
    if 'sibsp' in candidates:
        new_titanic = new_titanic.drop(['sibsp'], axis=1)

    if 'parch' in candidates:
...
# NOTE: THIS IS FOR PRESENTATION.
# Titanic data to a vector for K-Nearest Neighbors Algorithm.
def titanic_to_vector2(titanic_data) -> List[knn_vector]:
    """ Convert a data set into a list of knn_vectors.
    ...
    ag, pc = new_titanic['age'].to_numpy(), new_titanic['pclass'].to_numpy()

    ag_std = (ag - ag.min(axis=0)) / (ag.max(axis=0) - ag.min(axis=0))
    ag_scaled = ag_std * (1 - 0) + 0

    new_titanic['age'] = ag_scaled

    pc_std = (pc - pc.min(axis=0)) / (pc.max(axis=0) - pc.min(axis=0))
    pc_scaled = pc_std * (1 - 0) + 0

    new_titanic['pclass'] = pc_scaled

    # Convert the dataset to rows into KNN vectors.
    rows = list(new_titanic.to_records(index=False))
    results = [knn_vector(3, list(r)) for r in rows]

    # Return the results.
    return results


# KNN Prediction Function [NOTE: PRESENTATION]
def titanic_KNN2(kmodel, kvects, verbose=False) -> None:
    """K-Nearest Neighbors Prediction.
...
# Titanic prediction function [NOTE: PRESENTATION]
def titanic_predictions2(model=None, test_vects=None, verbose=False) -> None:
    """ Predicts Titanic Data based on model presented.
    ...
    if model.name == 'kNN_Model':
        ...
        score = titanic_KNN2(kmodel=model, kvects=test_vects, verbose=verbose)
